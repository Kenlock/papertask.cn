...
class Fre_ProjectAction extends AE_PostAction
{
    ...
    function post_sync() {
        ...
        if (!is_wp_error($result)) {
			$sourcetext=strip_tags($request['source_text']);
			//echo "<pre>";print_r($sourcetext);exit;
			...
			{
				add_post_meta( $result->ID, 'translategraphics', $translategraphics );
			}
			
			if($sourcetext!='')
			{
				add_post_meta( $result->ID, 'souce_text', $sourcetext );
			}
		else
		{
