...
<ul class="list-history-profile">
	...
	while (have_posts()) { the_post();
		...
		$sourcetext = get_post_meta ( $convert->ID, $key = 'source_text', $single = false );
		$translategraphics = get_post_meta ( $convert->ID, $key = 'translategraphics', $single = false );

		//if($sourcetext && $sourcetext[0]!=''){
			$convert->source_text = $sourcetext[0];
		//}
		//else
		//if($translategraphics && $translategraphics[0]!=''){
			$convert->translategraphics = $translategraphics[0];
		//}
		$postdata[] = $convert;
