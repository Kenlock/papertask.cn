...
$convert_tmp        = $post_object->convert($post);

//var_dump($convert_tmp->bid_budget_text);exit;
...
if($q_bid->have_posts()):
	while($q_bid->have_posts()): $q_bid->the_post();
		//if($post->post_status == "accept" || $post->post_status == "close") {
			$po = $ae_post_factory->get(BID);
			$cn = $po->convert($post);
			$accepted_bid_price = $cn->bid_budget_text;
			$bid_budget = $cn->bid_budget;
			break;
		//}
	endwhile;
endif;
...
if($role == 'administrator'){
 ...
 $total = $bid_budget - $commission_fee;

 $accepted_bid_price = fre_price_format($total); //$cn->bid_budget_text;
	...
	<div class="tab-content-project">
    	...
    	<div class="row title-tab-project">
            ...
            <div class="col-lg-2 col-md-1 col-sm-2 hidden-xs">
				...
				<span><?php _e("PROJECT PRICE", ET_DOMAIN); ?></span>
				<?PHP } ?>
            </div>
        </div>
        <!-- Title / End -->
            ...
            <div class="project type-project project-item">
                <div class="row">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-3 hidden-xs">
						...
						<?PHP if($project->post_status != "publish") {
									?>
                        <span class="budget-project-item"> <?php echo $accepted_bid_price; ?></span>
						<?PHP } ?>
                    </div>
                    ...
                    if(current_user_can( 'manage_options' ) && $project_status != 'close') {
											...
											//var_dump("thiasdadasd");var_dump($role);
                    ...
                    <?php } else {
                        ...
                        switch ($project_status) {
                            case 'publish':
                                if( ( fre_share_role() || $role == FREELANCER || $role == 'FREELANCER_VERIFIED' ) && $user_ID != $project->post_author ){
                            ...
                            case 'close':
                                if( (int)$project->post_author == $user_ID){ ?>
                                    ...
                                    <a title="<?php _e('Close',ET_DOMAIN);?>" href="#" id="<?php the_ID();?>"   class="btn btn-apply-project-item btn-project-status btn-close-project btn-danger" >
                                ...
                                ?>
                                    <a title="<?php  _e('Quit',ET_DOMAIN);?>" href="#" id="<?php the_ID();?>"   class="btn btn-apply-project-item btn-project-status btn-quit-project btn-danger" >
