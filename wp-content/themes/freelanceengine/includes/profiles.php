...
/**
* Registers a new post type verification
* @uses $wp_post_types Inserts new post type object into the list
*
* @param string  Post type key, must not exceed 20 characters
* @param array|string  See optional args description above.
* @return object|WP_Error the registered post type object, or an error object
*/
function fre_register_verification() {

    $labels = array(
        'name'                => __( 'Verification IDs', ET_DOMAIN ),
        'singular_name'       => __( 'Verification ID', ET_DOMAIN ),
        'add_new'             => _x( 'Add New Verification ID', ET_DOMAIN, ET_DOMAIN ),
        'add_new_item'        => __( 'Add New Verification ID', ET_DOMAIN ),
        'edit_item'           => __( 'Edit Verification ID', ET_DOMAIN ),
        'new_item'            => __( 'New Verification ID', ET_DOMAIN ),
        'view_item'           => __( 'View Verification ID', ET_DOMAIN ),
        'search_items'        => __( 'Search Verification ID', ET_DOMAIN ),
        'not_found'           => __( 'No Verification ID found', ET_DOMAIN ),
        'not_found_in_trash'  => __( 'No Verification IDs found in Trash', ET_DOMAIN ),
        'parent_item_colon'   => __( 'Parent Verification ID:', ET_DOMAIN ),
        'menu_name'           => __( 'Verifications', ET_DOMAIN ),
    );
    ...
    $args = array(
        ...
        'publicly_queryable'  => true,
        'exclude_from_search' => false,
        'has_archive'         => ae_get_option('fre_verification_archive', 'verifications'),
        'query_var'           => true,
        'can_export'          => true,
        'rewrite'             => array('slug' => ae_get_option('fre_verification', 'verification')),
        'capability_type'     => 'post',
        'supports'            => array(
                                'title', 'editor', 'author', 'thumbnail',
                                'excerpt','custom-fields', 'trackbacks', 'comments',
    ...
    global $ae_post_factory;
    $ae_post_factory->set(VERIFICATION, new AE_Posts( VERIFICATION, array(), array('verification_link') ) );
    register_post_type( VERIFICATION, $args );
}
add_action('init', 'fre_register_verification' );
...
class Fre_ProfileAction extends AE_PostAction {
    ...
    public function filter_query_args($query_args){
        ...
        if(isset($_REQUEST['query'])) {
            ...
            if(isset($query['sourcelanguage']) && $query['sourcelanguage'] != '') {
                $query_args['sourcelanguage'] = $query['sourcelanguage'];
            }

            if(isset($query['targetlanguage']) && $query['targetlanguage'] != '') {
                $query_args['targetlanguage'] = $query['targetlanguage'];
            }
        }
    ...
    function pre_get_profile($query)
    {
        if(is_post_type_archive('fre_profile')){
            ...
            if(isset($query_profile['post_type']) && $query_profile['post_type'] == PROFILE && current_user_can('manage_options')){
            ...
            if(isset($query_profile['post_type']) && $query_profile['post_type'] == PROFILE && is_post_type_archive(PROFILE) && !current_user_can( 'edit_others_posts' ) ){
                $query->query_vars['meta_query'][] = array(   'key' =>'user_available',
                                                            'value' => 'on',
                                                            'compare' => '='
                                                    );
    ...
    function sync_post() {
		...
		if(isset($request['current_user']) && (int)$request['current_user'] > 0) {
			if ($request['current_user'] != $user_ID && (ae_user_role($user_ID) == "administrator" && ae_user_role($user_ID) == 'editor') ) {
				$request['post_author'] = $request['current_user'];
				$Current_user = new WP_User( $request['current_user'] );
			}
		}
		if(isset($request['current_profile_id']) && (int) $request['current_profile_id'] > 0) {
			$request['method'] = "update";
			$request['id'] = $request['ID'] = $request['current_profile_id'];
		}
        ...
        if($request['method'] == 'create') {
            $profile_id = get_user_meta( /*$user_ID*/(int)$request['current_user'], 'user_profile_id', true );
            if($profile_id) {
...
class Fre_VerificationAction extends AE_PostAction {
    function __construct($post_type = 'verification') {
        $this->post_type = $post_type;
        $this->add_ajax('ae-fetch-verifications', 'fetch_post');
        $this->add_ajax('ae-verification-sync', 'sync_post');
        $this->add_filter('ae_convert_verification', 'ae_convert_verification');
    }
    
    function ae_convert_verification($result){

        $thumbnail_full_src              = wp_get_attachment_image_src( get_post_thumbnail_id( $result->ID ), 'full' );
        $thumbnail_src                   = wp_get_attachment_image_src( get_post_thumbnail_id( $result->ID ), 'verification' );
        $result->the_post_thumbnail_full = $thumbnail_full_src[0];
        $result->the_post_thumbnail      = $thumbnail_src[0];
    ...
    function sync_post() {
        ...
        $ae_users  = new AE_Users();
        $user_data = $ae_users->convert($current_user);
        $verification = $ae_post_factory->get($this->post_type);
        ...
        // set default post content
        $request['post_content'] = $request['this_post_content'];

        // sync place
		$result = $verification->sync($request);
        ...
        if (!is_wp_error($result)) {
            ...
            // action create profile
            if ($request['method'] == 'create') {
                $convert = $verification->convert($result);
                if( ae_get_option('switch_skill', false ) )
                {
                    $term = get_term_by( 'slug', $request['skill'], 'skill', ARRAY_A);
                    wp_set_post_terms($result->ID, $term, 'skill', true);
                ...
                $response = array(
                    'success'      => true,
                    'data'         => $convert,
                    'msg'          => __("Verification ID has been added successfully.", ET_DOMAIN)
                );
                wp_send_json($response);
            } else if($request['method'] == 'delete' || $request['method'] == 'remove' ){
                $response = array(
                    'success'      => true,
                    'msg'          => __("Verification ID has been deleted successfully.", ET_DOMAIN)
                );
                wp_send_json($response);
                //action update profile
            } else if($request['method'] == 'update'){
                ...
                $response = array(
                    ...
                    'msg'          => __("Verification ID has been updated successfully.", ET_DOMAIN)
                );
                wp_send_json($response);
            }
 ...
 * @since 1.0
 */
function fre_update_new_email($result, $user_data){
