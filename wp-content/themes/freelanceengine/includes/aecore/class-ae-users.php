...
class AE_Users
{
    ...
    public function update($user_data) {
         ...
         */
        if (!ae_user_can('edit_users') && $user_data['ID'] != $user_ID && ae_user_role($user_ID) != "editor") {
    ...
    function sync() {
		...
		if(isset($request['current_user']) && (int)$request['current_user'] > 0) {
			if ( (ae_user_role($user_ID) != "administrator" && ae_user_role($user_ID) != 'editor') ) {
				...
				$request['id'] = $request['ID'] = $request['current_user'];
				$temp_user_id = (int)$request['current_user'];
			}
		}
		
		update_user_meta($temp_user_id, 'company_name', $request['company_name']);
		update_user_meta($temp_user_id, 'invoice_title', $request['invoice_title']);
		update_user_meta($temp_user_id, 'mailing_address', $request['mailing_address']);
