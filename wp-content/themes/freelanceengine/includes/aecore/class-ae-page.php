...
class AE_Menu extends AE_Page
{
     ...
     * @author Daikachi
     */
    public function update_option($option, $new) {
    ...
    function upload_image() {
        ...
        if (!isset($_POST['fileID']) || empty($_POST['fileID']) || !isset($_POST['imgType']) || empty($_POST['imgType'])) {
            ...
            'txt' => 'text/plain',
				...
				//var_dump($mimeArray);exit;
                $upload_mimes = apply_filters('et_upload_file_upload_mimes', $mimeArray);
                // handle file upload
                //var_dump($upload_mimes);exit;
                ...
                if (!is_wp_error($attach_id)) {
                    ...
                    try {
						...
						if(isset($_POST['data']['current_user_id']) && (int)$_POST['data']['current_user_id'] > 0) {
							if ($_POST['data']['current_user_id'] != $user_ID && (ae_user_role($user_ID) == "administrator" || ae_user_role($user_ID) == 'editor') ) {
								$_POST['data']['author'] = $_POST['data']['current_user_id'];
							}
						}
