    ...
    */
    function freelancer_review_action() {
        ...
        */
         ...
         * check permission
        */
        //var_dump($user_ID);var_dump($author_bid);var_dump($bid_id_accepted);var_dump($project_id);exit;
        ...
        */
        ...
        if (!is_wp_error($comment)) {
			...
			/* Adding up the freelancer earned amount */
			$project     = get_post($project_id);
			$post_object = $ae_post_factory->get(PROJECT);
			$project     = $post_object->convert($project);
			$earned_from_this_project = (float) $project->et_budget;
			
			$totalEarned = $earned_from_this_project + (float) get_user_meta($author_bid, 'total_earned', true);
			if(ae_get_option('payer_of_commission', 'worker') === "worker") {
				$commission = (float) ae_get_option('commission', 0);
				if ($commission_type != 'currency' && $commission > 0) {
					$commission = ((float)($earned_from_this_project * $commission)) / 100;
				}
				$totalEarned -= $commission;
			}
			update_user_meta($author_bid, 'total_earned', $totalEarned);
