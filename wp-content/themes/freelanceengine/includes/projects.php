...
function fre_register_project() {
    ...
    /*$labels = array(
        'name' => _x('Payment Status', 'Taxonomy plural name', ET_DOMAIN) ,
        'singular_name' => _x('Payment Status', 'Taxonomy singular name', ET_DOMAIN) ,
        'search_items' => __('Search Payment Status', ET_DOMAIN) ,
        'popular_items' => __('Popular Payment Status', ET_DOMAIN) ,
        'all_items' => __('All Payment Status', ET_DOMAIN) ,
        'parent_item' => __('Parent Payment Status', ET_DOMAIN) ,
        'parent_item_colon' => __('Parent Payment Status', ET_DOMAIN) ,
        'edit_item' => __('Edit Payment Status', ET_DOMAIN) ,
        'update_item' => __('Update Payment Status', ET_DOMAIN) ,
        'add_new_item' => __('Add New Payment Status', ET_DOMAIN) ,
        'new_item_name' => __('New Payment Status Name', ET_DOMAIN) ,
        'add_or_remove_items' => __('Add or remove Payment Status', ET_DOMAIN) ,
        'choose_from_most_used' => __('Choose from most used enginetheme', ET_DOMAIN) ,
        'menu_name' => __('Payment Status', ET_DOMAIN) ,
    );
    ...
    $args = array(
        ...
        'query_var' => true,
        'rewrite' => array(
            'slug' => ae_get_option('payment_status_slug', 'payment_status') ,
            'hierarchical' => false
        ) ,
        'query_var' => true,
        'capabilities' => array(
    ...
    register_taxonomy('payment_status', array(
        PROJECT
    ) , $args);
*/
    ...
    add_action( 'manage_project_posts_custom_column', 'populate_columns' );
    add_filter('views_edit-project','sort_payment_status');
    //var_dump(new AE_Posts(PROJECT, $project_tax, $project_meta));exit;
}
function sort_payment_status(){
...
function my_filter_list() {
    ...
    if ( $screen->post_type == 'project' ) {
      ...
      <?php
          $current_v = isset($_GET['payment_status'])? $_GET['payment_status']:'';
...
function populate_columns( $column ) {
    if ( 'payment_status' == $column ) {
      ...
      $bid_id_accepted = get_post_meta(get_the_ID(), 'accepted', true);
        $payment_status = esc_html( get_post_meta( $bid_id_accepted, 'payment_status', true ) );
...
function perform_filtering( $query ) {
    $qv = &$query->query_vars;
    $type = $_GET['post_type'];
    if ( 'project' == $type && isset($_GET['ADMIN_FILTER_FIELD_VALUE']) && $_GET['ADMIN_FILTER_FIELD_VALUE'] != '') {
...
class Fre_ProjectAction extends AE_PostAction
{
    function __construct($post_type = 'project') {
        ...
        $this->add_action('ae_after_update_order', 'mail_after_update_order');
    ...
    public function filter_query_args($query_args) {
        ...
        */
        ...
        if (isset($query['is_profile']) && $query['is_profile'] == 'true') {
            ...
            if (!isset($query['post_status']) || $query['post_status'] == '') {
                $query_args['post_status'] = array(
                    'pending','publish','close', 'complete', 'disputing'
                );
            }else{
                $query_args['post_status'] = $query['post_status'];
            }
    ...
    function post_sync() {
        ...
        ## Add meta for translate graphic yes/no
        ## By FASTGuru - https://freelaner.com/u/FastGuru.html
        $translategraphics=@$request['translategraphics'];
        //var_dump($translategraphics);exit;
        if($translategraphics!='' && $translategraphics!=null) {
            if ( ! add_post_meta( $result->ID, 'translategraphics', 'Yes',true) ) {
               update_post_meta($result->ID, 'translategraphics', 'Yes');
            }
        ...
        /*if ( ! add_post_meta( $result->ID, 'source_text', $source_text,true) ) {
           update_post_meta($result->ID, 'source_text', $source_text );
        }*/

            ## Add meta for source_text
        ## By FASTGuru - https://freelancer.com/u/FastGuru.html
        $source_text=@$request['source_text'];
        //var_dump($source_text);exit;
        //if($source_text!='') {
          if ( ! add_post_meta( $result->ID, 'source_text', $source_text,true) ) {
             update_post_meta($result->ID, 'source_text', $source_text );
          }
    ...
    public function validate_data($data) {
        ...
        if (!current_user_can('manage_options')) {
            ...
            ## Disabled the Budget field validation
            ## By FASTGuru - https://freelaner.com/u/FastGuru.html
...
?>
        ...
        if ($role == 'freelancer' || $role == 'FREELANCER_VERIFIED' ) {
?>
                <li>
            ...
            echo fre_count_user_posts_by_type($user_id, BID, 'complete'); ?>
                ...
                <li>
                    ...
                    <span class="addr-wrap">
                        <?php
            _e('Address:', ET_DOMAIN); ?>
                        <span class="info addr" title="<?php
