...
class Fre_MessageAction extends AE_Base
{
    ...
    function __construct() {
        ...
        // send message
        $this->add_ajax('ae-sync-message-chat-box', 'sendMessage');
        $this->add_ajax('ae-sync-message-translator-box', 'sendMessage');

        // get older message
        $this->add_ajax('ae-fetch-messages-chat-box', 'fetchMessage');
        $this->add_ajax('ae-fetch-messages-translator-box', 'fetchMessage');
    ...
    function sendMessage() {
		...
		if($_REQUEST['action'] == 'ae-sync-message-translator-box') {
			$boxType = 'translator_box';
		}
		else if($_REQUEST['action'] == 'ae-sync-message-chat-box') {
			$boxType = 'chat_box';
		}
        ...
        if ((!isset($_REQUEST['fileID']) || count((array)$_REQUEST['fileID']) == 0) ) {
        ...
        if (!is_wp_error($comment)) {
			...
			// add box type
			update_comment_meta( $comment->comment_ID, 'fre_comment_box_type', $boxType );
            ...
            $this->fre_update_project_meta($project, 1, $boxType);
    ...
    function fetchMessage() {
		...
		if($_REQUEST['action'] == 'ae-fetch-messages-translator-box') {
			$boxType = 'translator_box';
		}
		else if($_REQUEST['action'] == 'ae-fetch-messages-chat-box') {
			$boxType = 'chat_box';
		}
        ...
        */
        ...
        if (!empty($data)) {
            ...
            if( isset($query['use_heartbeat']) && $query['use_heartbeat'] == 1){
                ...
                if( $project ){
                    $this->fre_reset_project_meta($project, $boxType);
                }
            }
            wp_send_json($data);
        } else {
    ...
    public  function fre_send_data_to_heartbeat($response, $data){
        if( isset($data['new_message']) ){
            ...
            if( $post ){
                ...
                $val = $this->fre_get_project_meta($project); // returns the box_name if the new message found
     ...
     * @author Tambh
     */
    public  function fre_update_project_meta($project, $val, $boxType = 'chat_box'){
        global $user_ID;
        if( $user_ID == $project->post_author ){
            update_post_meta($project->ID, 'fre_freelancer_new_msg_'.$boxType, $val);
        }
        else{
            update_post_meta($project->ID, 'fre_employer_new_msg_'.$boxType, $val);
        }
    }
     /**
     ...
     * @author Tambh
     */
    public  function fre_reset_project_meta($project, $boxType = 'chat_box'){
        global $user_ID;
        if( $user_ID == $project->post_author ){
            update_post_meta($project->ID, 'fre_employer_new_msg_'.$boxType, 0);
        }
        else{
            update_post_meta($project->ID, 'fre_freelancer_new_msg_'.$boxType, 0);
        }
    }
    /**
     ...
     * @author Tambh
     */
    public function fre_get_project_meta($project){ // chat or translator
        global $user_ID;
        $val = 0;
		
		foreach(array("chat_box", "translator_box") as $boxType) {
			if( $user_ID == $project->post_author ){
				$val = get_post_meta($project->ID, 'fre_employer_new_msg_'.$boxType, true);
			}
			else{
				$val = get_post_meta($project->ID, 'fre_freelancer_new_msg_'.$boxType, true);
			}
