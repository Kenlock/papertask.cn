...
function fre_user_notification($user_id = 0, $page = 1) {
    ...
    while (have_posts()) {
        ...
        if($type=='new_private_message'){
            ...
            if ( !is_plugin_active( 'ae-private-message/ae_private_message.php' ) ) {
                continue;
            }
        }
 ...
 * @author Tuandq
 */
add_action('transition_comment_status', 'fre_approve_comment_callback', 10, 3);
function fre_approve_comment_callback($new_status, $old_status, $comment) {
    if($new_status != $old_status){
        if($new_status == 'approved'){
            ...
            $insert = $noti->insert($notification);
        }
    }
}
/**
 ...
 * @author Tuandq
 */
add_action('wp_insert_comment', 'fre_auto_approve_comment_callback', 11, 2);
