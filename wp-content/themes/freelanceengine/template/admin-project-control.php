...
if( current_user_can( 'manage_options' )) { ?>
    <?php
        if( $post->post_status == 'complete' ) { //&& ae_get_option('use_escrow')
            ...
            $order = get_post_meta($bid_id_accepted, 'payment_status', true);
            if($order != 1) :
        ?>
			<?PHP 
        ...
        <ul class="button-event event-listing">
            ...
            <?php }else { ?>
                <li class="tooltips remove archive"><a class="action" data-action="archive" data-toggle="tooltip" title="<?php _e("Archive", ET_DOMAIN); ?>" data-original-title="<?php _e("Archive", ET_DOMAIN); ?>" href="#"><span class="icon fa fa-archive" data-icon="#"></span></a></li>
            <?php }
        } ?>
    </ul>
<?php } ?>
