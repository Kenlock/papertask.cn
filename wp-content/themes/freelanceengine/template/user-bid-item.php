...
<li <?php post_class( 'user-bid-item' ); ?>>
    ...
    <?php if($bid->project_status == "publish"){ ?>
        ...
        <!-- danng !-->
        <?php }  else if($bid->project_status == 'complete' && ae_user_role() == 'freelancer' && $bid->ID == $bid->project_accepted ){ ?>
        <div class="col-md-6">
            <a href="<?php echo add_query_arg('review',1,$project->permalink) ?>" class="btn btn-apply-project-item">
                <?php _e("Review job", ET_DOMAIN) ?>
            </a>
        </div>
        <?php  } else if($bid->project_status == 'close' && ae_user_role() == 'freelancer' && $bid->ID == $bid->project_accepted  ){ ?>
        <div class="col-md-6">
