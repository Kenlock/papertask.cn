...
class ET_FreelanceEngine extends AE_Base
{
    function __construct() {
        ...
        if (!isset($wp_roles->roles[FREELANCER_VERIFIED])) {

            //all new roles
            add_role(FREELANCER_VERIFIED, __('Freelancer-Verified', ET_DOMAIN) , array(
                'read' => true,
         ...
         * add new role EMPLOYER
         */
        if (!isset($wp_roles->roles[EMPLOYER_VIP])) {
            add_role(EMPLOYER_VIP, __('Employer-VIP', ET_DOMAIN) , array(
                'read' => true,
        ...
        */
        ...
        add_image_size('verification', 230, 170, true);
        ...
        $this->verification_action = new Fre_VerificationAction();
		...
		$this->add_filter('cron_schedules', 'auto_review_cron_schedules');
	}

	function auto_review_cron_schedules($schedules) {
    ...
    function on_add_scripts() {
        ...
        */
        ...
        wp_localize_script('front', 'fre_fronts', array(
            ...
            'verification_img' => __('Please select an image!', ET_DOMAIN),
       ...
       $this->add_script('wow-scroll', get_template_directory_uri() . '/assets/js/index.js', array(
        ...
        // author page js
        if (is_author()) {
            $this->add_script('profile', get_template_directory_uri() . '/assets/js/profile.js', array() , ET_VERSION, true);
...
function auto_review_activation() {
	if (!wp_next_scheduled('auto_complete_project')) {
		wp_schedule_event( time(), '2min', 'auto_complete_project' );
	}
}
add_action('wp', 'auto_review_activation');

function project_complete_function() {
	global $wpdb;
	$query = 
	"SELECT DISTINCT comment_post_ID as id FROM {$wpdb->comments}
		WHERE comment_ID IN (SELECT comment_id FROM {$wpdb->commentmeta} WHERE meta_key='fre_comment_box_type' AND meta_value = 'translator_box')
		AND ABS(DATEDIFF(comment_date, NOW())) >= 7
		AND comment_type = 'message'
		AND comment_post_ID IN (SELECT ID FROM {$wpdb->posts} WHERE post_type = 'project' AND post_status <> 'complete')";
	...
	foreach($results as $result) {
		...
		$commentdata = array(
			...
			'comment_author' => get_the_author_meta('display_name', $post->post_author),
			'comment_author_email' => get_the_author_meta('user_email', $post->post_author),
			'comment_author_url' => 'http://',
			'comment_content' => '',
			'comment_type' => 'em_review',
			'comment_parent' => 0,
		...
		$commentdata = apply_filters( 'ae_filter_data_em_review', $commentdata );
		/**
		...
		if (!is_wp_error($comment)) {
			// update comment meta
			update_comment_meta($comment, 'et_rate', 5);
			// do action after insert a comment with type args
			do_action( 'ae_filter_data_em_review' , $comment );
			
			
			//$action = new Fre_ReviewAction();
			//$action->update_after_empoyer_review($post->ID, $comment);
			...
			if (isset($args['score']) && $args['score']) {
				$rate = (float)$args['score'];
				if ($rate > 5) $rate = 5;
			...
			$mail->review_freelancer_email($post->ID);
			 ...
			 * @author Dakachi
			 */
			do_action('fre_complete_project', $post->ID, $args);
			
		}
	}
}
add_action('auto_complete_project', 'project_complete_function');