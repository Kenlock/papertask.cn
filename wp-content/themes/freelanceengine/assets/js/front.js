(function($, Models, Collections, Views) {
	$(document).ready(function() {
		...
		Models.Verification = Backbone.Model.extend({
			action: 'ae-verification-sync',
			initialize: function() {}
		});
		/**
		 * Verification collection
		 */
		Collections.Verifications = Backbone.Collection.extend({
			model: Models.Verification,
			action: 'ae-fetch-verifications',
			initialize: function() {
				this.paged = 1;
			}
		});
		/**
		 * Define verification item view
		 */
		VerificationItem = Views.PostItem.extend({
			events: {
				'click a.delete' : 'removeVerification'
			},
			initialize: function(){
				this.blockUi    = new Views.BlockUi();
			},
			tagName: 'li',
			className: 'verification-item col-md-4',
			template: _.template($('#ae-portfolio-loop').html()),
			removeVerification: function(event){
				event.preventDefault();
				var view = this;
				this.model.destroy({
					beforeSend: function() {
		...
		/**
		 * List view control Verifications list
		 */
		ListVerifications = Views.ListPost.extend({
			tagName: 'li',
			itemView: VerificationItem,
			itemClass: 'verification-item'
		});
			...
			handleAuth: function(model, resp, jqXHR) {
				...
				if (resp.success) {
					...
					if(data.do == "login" && !ae_globals.is_submit_project){
						...
						if( window.location.href == data.redirect_url || data.redirect_url == ae_globals.homeURL ){
	        ...
	        setupFields: function() {
				...
				if (typeof view.carousels === 'undefined') {
	                view.carousels = new Views.Carousel({
	                    ...
	                    model: view.model,
	                    extensions : 'txt,gif,jpeg,jpg,png,doc,docx,ppt,pptx,pdf,chm,xls,xlsx,zip,rar'
	                });
	            } else {
		...
		Views.SubmitProject = Views.SubmitPost.extend({
	        onAfterInit: function() {
	            ...
	            view.carousels = new Views.Carousel({
	                el: $('#gallery_container'),
	                model: view.model,
	                extensions : 'pdf,doc,docx,png,jpg,gif,zip,xls,xlsx'
	            });
