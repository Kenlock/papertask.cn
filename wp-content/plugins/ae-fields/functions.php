...
add_action('wp_enqueue_scripts', 'ae_field_script');
function ae_field_script() {

    wp_enqueue_script('ae_field', plugin_dir_url(__FILE__) . '/assets/field.js', array(
            'underscore',
            'backbone',
            'appengine'
        ) , '1.0', true);
    ...
    $date_format =  str_replace(' ','/',str_replace(array_keys($replace) , array_values($replace) , get_option('date_format')));
    wp_localize_script('ae_field',"date_format",$date_format);
}
?>
